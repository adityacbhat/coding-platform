...
def isAnagram(s: str, t: str) -> bool:
  ...
  },
  {
    slug: 'two-pointers',
    title: 'Two Pointers',
    description: 'Use two indices to traverse data in a coordinated way—converging, same-direction, read/write, merge, partition, or monotonic—achieving O(n) solutions instead of nested loops.',
    position: { x: 220, y: 260 },
    dependencies: ['arrays-hashing'],
    subConcepts: [
      {
        slug: 'opposite-ends',
        title: 'Converging Pointers (Opposite Ends)',
        description: 'l = 0, r = n-1, move inward. Comparing/choosing from both ends using some rule (often sorted / symmetric).',
        whenToUse: [
          'Palindrome checks',
          'Sorted 2-sum',
          'Container problems',
          'Minimizing/maximizing something with ends',
      ...
      },
      {
        ...
        title: 'Read/Write (In-place Filtering / Compaction)',
        description: 'Read scans every element; write places "kept" elements. For removing elements, compressing arrays, moving zeros, deduping in-place.',
        whenToUse: [
          '"In-place"',
          '"O(1) extra space"',
          '"Remove duplicates"',
      ...
      {
        slug: 'same-direction',
        title: 'Fast/Slow (Tortoise–Hare)',
        description: 'Two pointers move forward at different speeds (slow += 1, fast += 2 or similar). For cycle detection, middle of list, kth from end, duplicate detection via cycles.',
        whenToUse: [
          'Linked list cycles',
          '"Find middle"',
          '"Detect loop"',
      ...
      {
        slug: 'merge-pointers',
        title: 'Multiple-Streams Merge Pointers (Two Sorted Inputs)',
        description: 'You have two (or more) sorted streams (arrays/lists/iterators). Maintain pointers i (in A) and j (in B). At each step, take the smaller (or needed) element and advance the pointer that produced it. This gives O(n + m) instead of nested loops.\n\nWhy this works (the sorted guarantee): A[i] < B[j] → A[i] is the smallest remaining across both, so it must come next and cannot match any earlier B values. A[i] > B[j] → same for B[j]. A[i] == B[j] → overlap/match (intersection) or duplicate (union). The sorted property gives monotonic progress: pointers never move backward.\n\nSub-patterns: (1) Classic Merge — produce sorted output; if A[i] <= B[j] take A[i] else take B[j], append leftovers. (2) Intersection/Union — if equal add once and advance both; if smaller advance smaller side. (3) Two-Timeline Scan — advance whichever interval/event ends first. (4) Counting — count matches/pairs using monotonic sweep.\n\nCore rule: At each step, advance the pointer pointing to the element that cannot help you anymore. Smaller value can\'t match anything earlier in the other array. Earlier-ending interval can\'t overlap anything later.',
        whenToUse: [
          'Two sorted inputs: "sorted array", "sorted list", "two sorted linked lists", "sorted intervals"',
          'Need to combine/compare: merge into one, find intersection/union, count pairs across arrays, compute difference (A - B)',
          '"Next event among two timelines" — matching events, interval overlaps, compare logs/schedules',
          'Want O(n+m) — naive approach would compare many pairs; sorted order avoids that',
          'Merge K Sorted Lists extends this idea with a heap',
        ],
        problems: [
          {
            title: 'Core Template: Merge Two Sorted Arrays',
            difficulty: 'Easy',
            explanation: 'The fundamental merge loop. Pointers i and j scan A and B. At each step, take the smaller element, append to output, advance that pointer. When one list is exhausted, append the rest of the other. Time O(n+m), space O(n+m) for output. Can be O(1) if merging in-place into a buffer.',
            examples: [
              { input: 'A = [1, 3, 5], B = [2, 4, 6]', output: '[1, 2, 3, 4, 5, 6]', explanation: 'Interleave by always taking the smaller of A[i] and B[j].' },
              { input: 'A = [1, 2], B = [3, 4, 5]', output: '[1, 2, 3, 4, 5]', explanation: 'A exhausted first; append B[j:].' },
            ],
            codeLines: [
              { code: 'i = j = 0', explanation: 'Pointers at start of each array' },
              { code: 'out = []', explanation: 'Output list' },
              { code: 'while i < len(A) and j < len(B):', explanation: 'While both have elements' },
              { code: '    if A[i] <= B[j]:', explanation: 'A[i] is smaller (or equal — take A for stability)' },
              { code: '        out.append(A[i]); i += 1', explanation: 'Take from A, advance i' },
              { code: '    else:', explanation: 'B[j] is smaller' },
              { code: '        out.append(B[j]); j += 1', explanation: 'Take from B, advance j' },
              { code: '# leftovers', explanation: 'One list exhausted' },
              { code: 'out.extend(A[i:])', explanation: 'Append rest of A (or nothing if i >= len(A))' },
              { code: 'out.extend(B[j:])', explanation: 'Append rest of B (or nothing if j >= len(B))' },
            ],
            recognitionCues: ['"merge"', '"two sorted"', '"combine sorted"'],
            corePattern: 'if A[i] <= B[j] take A[i] and i++; else take B[j] and j++. Append leftovers.',
            mentalChecklist: [
              'Are both inputs sorted (or can I sort)? → Merge pointers',
              'What am I producing? Merged list → classic merge; intersection → equality + advance both; union → take smaller, skip dupes',
              'When equal: merge all → take one, advance one (or both for intersection); union unique → take once, advance both',
              'Space constraints? In-place + buffer → backwards fill from end',
            ],
          },
          {
            title: 'In-Place Variant: Merge Sorted Array (LC 88)',
            difficulty: 'Easy',
            explanation: 'Merge B into A in-place. A has extra trailing space. Key trick: fill from the end so you never overwrite unprocessed values. Put the largest remaining element at k, then k-1, etc. The largest remaining must go at the end. Time O(m+n), space O(1).',
            examples: [
              { input: 'nums1 = [1,2,3,0,0,0], m=3, nums2 = [2,5,6], n=3', output: '[1,2,2,3,5,6]', explanation: 'Merge B into A without extra array.' },
              { input: 'nums1 = [2,0], m=1, nums2 = [1], n=1', output: '[1,2]', explanation: 'Fill from end: 2 goes to index 1, 1 to index 0.' },
            ],
            codeLines: [
              { code: 'i = m - 1', explanation: 'Last real element in A' },
              { code: 'j = n - 1', explanation: 'Last element in B' },
              { code: 'k = m + n - 1', explanation: 'Write index from the end' },
              { code: 'while j >= 0:', explanation: 'Process all of B (when j < 0, A\'s rest is already in place)' },
              { code: '    if i >= 0 and A[i] > B[j]:', explanation: 'A[i] is larger' },
              { code: '        A[k] = A[i]; i -= 1', explanation: 'Take from A, move i left' },
              { code: '    else:', explanation: 'B[j] is larger or A exhausted' },
              { code: '        A[k] = B[j]; j -= 1', explanation: 'Take from B, move j left' },
              { code: '    k -= 1', explanation: 'Move write position left' },
            ],
            recognitionCues: ['"merge in-place"', '"A has extra space"', '"O(1) space"'],
            corePattern: 'Fill from end. Largest remaining goes at k. Never overwrite unprocessed.',
            mentalChecklist: [
              'In-place and one array has buffer? → Backwards fill',
              'Why from end? Unprocessed elements are at indices < k, so we never overwrite them',
              'When j < 0, A\'s remaining elements are already in correct positions',
            ],
          },
          {
            title: 'Merge Two Sorted Lists',
            difficulty: 'Easy',
            explanation: 'Two linked lists already sorted. Use pointers p1, p2; build a new list by always attaching the smaller node. Classic merge on linked lists. Recognition: "merge", "two sorted linked lists".',
            examples: [
              { input: 'list1 = [1, 2, 4], list2 = [1, 3, 4]', output: '[1, 1, 2, 3, 4, 4]', explanation: 'Merge into one sorted linked list.' },
              { input: 'list1 = [], list2 = [0]', output: '[0]', explanation: 'One list empty, result is the other.' },
            ],
            codeLines: [
              { code: 'dummy = ListNode()', explanation: 'Dummy head to simplify edge cases' },
              { code: 'curr = dummy', explanation: 'Build result from curr' },
              { code: 'while l1 and l2:', explanation: 'While both have nodes' },
              { code: '    if l1.val <= l2.val:', explanation: 'Take smaller, advance its pointer' },
              { code: '        curr.next = l1; l1 = l1.next', explanation: 'Attach l1, move l1' },
              { code: '    else:', explanation: 'l2 is smaller' },
              { code: '        curr.next = l2; l2 = l2.next', explanation: 'Attach l2, move l2' },
              { code: '    curr = curr.next', explanation: 'Advance output pointer' },
              { code: 'curr.next = l1 or l2', explanation: 'Append remaining (non-empty list)' },
              { code: 'return dummy.next', explanation: 'Return merged list' },
            ],
            recognitionCues: ['"merge"', '"two sorted linked lists"'],
            corePattern: 'At each step, attach smaller node and advance its pointer. Append leftovers.',
            mentalChecklist: ['Both inputs sorted? → Merge pointers', 'Producing merged list? → Classic merge, always take smaller'],
          },
          {
            title: 'Intersection of Two Arrays II',
            difficulty: 'Medium',
            explanation: 'Find common elements with counts. Sort both (or they\'re already sorted). If A[i] == B[j]: record match, advance both. Else advance the smaller. Set-like ops on sorted arrays — intersection means equality + advance both. If union unique: always take smaller; if equal take once and advance both, skip duplicates within each list.',
            examples: [
              { input: 'nums1 = [1, 2, 2, 1], nums2 = [2, 2]', output: '[2, 2]', explanation: 'Common elements with multiplicity.' },
              { input: 'nums1 = [4, 9, 5], nums2 = [9, 4, 9, 8, 4]', output: '[4, 9] or [9, 4]', explanation: 'Intersection; order may vary.' },
            ],
            codeLines: [
              { code: 'nums1.sort(); nums2.sort()', explanation: 'Sort first if not already sorted' },
              { code: 'i = j = 0', explanation: 'Merge-style pointers' },
              { code: 'out = []', explanation: 'Collect matches' },
              { code: 'while i < len(nums1) and j < len(nums2):', explanation: 'Scan both' },
              { code: '    if nums1[i] == nums2[j]:', explanation: 'Match found' },
              { code: '        out.append(nums1[i]); i += 1; j += 1', explanation: 'Record and advance both' },
              { code: '    elif nums1[i] < nums2[j]:', explanation: 'A[i] too small to match' },
              { code: '        i += 1', explanation: 'Advance i' },
              { code: '    else:', explanation: 'B[j] too small' },
              { code: '        j += 1', explanation: 'Advance j' },
              { code: 'return out', explanation: 'Return intersection with counts' },
            ],
            recognitionCues: ['"common elements"', '"intersection"', '"counts"'],
            corePattern: 'If equal → record and advance both. Else advance smaller.',
            mentalChecklist: [
              'Set-like ops on sorted arrays? → Merge pointers',
              'When equal: record match, advance both',
              'Union unique: skip duplicates in each list as you scan',
            ],
          },
          {
            title: 'Interval List Intersections',
            difficulty: 'Medium',
            explanation: 'Two lists of disjoint, sorted intervals. Overlap exists if max(start) <= min(end). Advance the interval that ends first — once it ends, it can\'t overlap any future intervals from the other list. This is merge pointers where elements are intervals: "advance whichever ends first". Used in Meeting room overlap, compare logs/schedules.',
            examples: [
              { input: 'firstList = [[0,2],[5,10],[13,23]], secondList = [[1,5],[8,12],[15,24]]', output: '[[1,2],[5,5],[8,10],[15,23]]', explanation: 'Overlapping intervals.' },
              { input: 'firstList = [[1,3],[5,9]], secondList = []', output: '[]', explanation: 'No second list, no intersections.' },
            ],
            codeLines: [
              { code: 'i = j = 0', explanation: 'Pointers per list' },
              { code: 'out = []', explanation: 'Collect overlaps' },
              { code: 'while i < len(A) and j < len(B):', explanation: 'Merge-style scan' },
              { code: '    lo = max(A[i][0], B[j][0])', explanation: 'Overlap start' },
              { code: '    hi = min(A[i][1], B[j][1])', explanation: 'Overlap end' },
              { code: '    if lo <= hi:', explanation: 'Valid overlap' },
              { code: '        out.append([lo, hi])', explanation: 'Record intersection' },
              { code: '    if A[i][1] < B[j][1]:', explanation: 'A ends first' },
              { code: '        i += 1', explanation: 'Advance A — it can\'t overlap later B intervals' },
              { code: '    else:', explanation: 'B ends first (or tie)' },
              { code: '        j += 1', explanation: 'Advance B' },
              { code: 'return out', explanation: 'Return all intersections' },
            ],
            recognitionCues: ['"two interval lists"', '"find intersections"', '"overlap"'],
            corePattern: 'Overlap = max(start) <= min(end). Advance the interval that ends first.',
            mentalChecklist: [
              'Two timelines/intervals? → Merge pointers',
              'Earlier-ending interval can\'t overlap later ones → move it',
              'Compare logs/schedules, meeting room overlap variants',
            ],
          },
        ],
      },
      {
        slug: 'partition-pointers',
        title: 'Partition Pointers (Two-Way / Three-Way Partition)',
        description: 'Pointers rearrange elements around a pivot or categories (often with swaps). For partitioning like quicksort, Dutch National Flag (0/1/2), segregating negatives/positives.',
        whenToUse: [
          '"Rearrange in-place"',
      ...
      {
        slug: 'monotonic-two-pointers',
        title: 'Monotonic Two Pointers (Count Pairs / Valid Ranges)',
        description: 'Pointers move forward with a monotonic relationship, never moving backward; often counts efficiently. For counting pairs or ranges using sorted order or monotonic properties (avoid nested loops).',
        whenToUse: [
          '"Count pairs with sum ≤ X"',
          '"Number of subarrays with condition" (sometimes after sorting)',
          'Counting valid ranges in sorted data',
        ],
        problems: [],
      },
    ],
