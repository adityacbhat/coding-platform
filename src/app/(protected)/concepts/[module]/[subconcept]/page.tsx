  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center gap-2 text-sm text-slate-400 mb-6">
        <Link href="/concepts" className="hover:text-indigo-600 transition-colors">
          Concepts
        </Link>
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
        </svg>
        <Link href={`/concepts/${module.slug}`} className="hover:text-indigo-600 transition-colors">
          {module.title}
        </Link>
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
        </svg>
        <span className="text-slate-700">{subConcept.title}</span>
      </div>

      <div className="mb-10">
        <h1 className="text-4xl font-bold text-slate-800 mb-3">{subConcept.title}</h1>
        <div className="text-xl text-slate-500 space-y-4">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-violet-50 border border-indigo-200 rounded-2xl p-6 mb-10">
        ...
        <ul className="space-y-3">
          {subConcept.whenToUse.map((use, i) => (
            <li key={i} className="flex items-start gap-3 text-slate-600">
              <span className="w-6 h-6 rounded-full bg-indigo-100 border border-indigo-200 flex items-center justify-center flex-shrink-0 mt-0.5">
      ...
      {subConcept.pythonExamples ? (
        <div className="mb-10">
          <h2 className="text-2xl font-bold text-slate-800 mb-6">Code Examples</h2>
          <div className="space-y-6">
            {subConcept.pythonExamples.map((example, i) => (
              <div key={i} className="space-y-2">
                ...
                <p className="text-sm text-slate-500 italic pl-1">{example.explanation}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <>
          <div className="mb-10">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-slate-800">Sample Problems</h2>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 mb-10">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                  <h4 className="text-sm font-semibold text-emerald-600 mb-2">Pattern Recognition</h4>
                ...
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                  <h4 className="text-sm font-semibold text-amber-600 mb-2">First Thought</h4>
                  <p className="text-sm text-slate-500">
      ...
      <div className="flex items-center justify-between pt-6 border-t border-slate-200">
        {prevSubConcept ? (
          <Link
            ...
            className="group flex items-center gap-3 text-slate-400 hover:text-slate-800 transition-colors"
          >
        ...
        {nextSubConcept ? (
          <Link
            ...
            className="group flex items-center gap-3 text-slate-400 hover:text-slate-800 transition-colors text-right"
          >
            <div>
