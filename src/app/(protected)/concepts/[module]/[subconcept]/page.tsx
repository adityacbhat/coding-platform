  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center gap-2 text-sm text-slate-500 mb-6">
        ...
        <Link href={`/concepts/${module.slug}`} className="hover:text-indigo-400 transition-colors">
          {module.title}
        </Link>
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
      ...
      <div className="mb-10">
        <h1 className="text-4xl font-bold text-slate-100 mb-3">{subConcept.title}</h1>
      ...
      <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-6 mb-10">
        ...
        <ul className="space-y-3">
          {subConcept.whenToUse.map((use, i) => (
            <li key={i} className="flex items-start gap-3 text-slate-300">
              <span className="w-6 h-6 rounded-full bg-indigo-500/30 border border-indigo-500/50 flex items-center justify-center flex-shrink-0 mt-0.5">
                <span className="text-xs text-indigo-300 font-semibold">{i + 1}</span>
              </span>
      ...
      {subConcept.pythonExamples ? (
        <div className="mb-10">
          <h2 className="text-2xl font-bold text-slate-100 mb-6">Code Examples</h2>
        ...
        <>
          <div className="mb-10">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-slate-100">Sample Problems</h2>
          ...
          {subConcept.problems.length > 0 && (
            <div className="bg-slate-800/60 border border-slate-700 rounded-2xl shadow-sm p-6 mb-10">
              <h3 className="text-lg font-semibold text-slate-100 mb-4">Key Takeaways</h3>
      ...
      <div className="flex items-center justify-between pt-6 border-t border-slate-700">
        {prevSubConcept ? (
          <Link
            ...
            className="group flex items-center gap-3 text-slate-500 hover:text-slate-100 transition-colors"
          >
        ...
        {nextSubConcept ? (
          <Link
            ...
            className="group flex items-center gap-3 text-slate-500 hover:text-slate-100 transition-colors text-right"
          >
            <div>
