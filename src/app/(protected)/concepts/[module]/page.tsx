...
export default async function ModulePage({ params }: { params: Promise<{ module: string }> }) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <Link
        href="/concepts"
        className="text-indigo-400 hover:text-indigo-300 font-medium text-sm mb-6 inline-flex items-center gap-1 transition-colors"
      >
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-3">
          <h1 className="text-4xl font-bold text-slate-100">{module.title}</h1>
          {isPlaceholder && (
      ...
      {prerequisites.length > 0 && (
        <div className="bg-slate-800/60 border border-slate-700 rounded-2xl shadow-sm p-5 mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {prerequisites.map((prereq) => (
              <Link
                ...
                className="px-3 py-1.5 bg-slate-700/80 hover:bg-slate-600 text-slate-300 rounded-xl text-sm transition-colors border border-slate-600"
              >
                {prereq!.title}
              </Link>
            ))}
      ...
      {isPlaceholder ? (
        <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-12 text-center shadow-sm">
          <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-slate-700 flex items-center justify-center">
            <svg className="w-8 h-8 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
            </svg>
          </div>
          <h3 className="text-xl font-semibold text-slate-100 mb-2">Content Coming Soon</h3>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-slate-100">Patterns & Techniques</h2>
          ...
          <div className="grid gap-4">
            {module.subConcepts.map((subConcept, index) => (
              <Link
                ...
                className="group block bg-slate-800/60 border border-slate-700 rounded-2xl p-6 hover:border-indigo-500/50 hover:shadow-md shadow-sm card-hover transition-all duration-200"
                style={{
